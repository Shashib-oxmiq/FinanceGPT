...
- 2026-06 (fork): Milestone Reminders — GET /api/reminders aggregates expiring documents (from extracted metadata.expiry_date), insurance renewals (maturity_date within 90d) and tracked life-event milestones with missing docs. Life-event tracking: POST /api/life-events/track, GET /api/life-events/tracked, DELETE /api/life-events/track/{event}. New /reminders page + sidebar nav item with live count badge; "Remind me / Tracking" toggle on Life Event guides. Document classification now also extracts expiry_date. Curl + screenshot verified.
