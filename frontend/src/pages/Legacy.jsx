  ...
  return (
    <Page>
      <PageHeader
        ...
        title="Next-of-Kin & Legacy"
        subtitle="Designate who can access your records, and generate a complete handover pack so benefits pass on correctly."
        actions={
          <button onClick={() => setShowForm(true)} data-testid="add-contact" className="flex items-center gap-2 bg-primary text-primary-foreground px-4 py-2.5 rounded-md text-sm font-semibold hover:opacity-90 transition-opacity">
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 space-y-4">
          <div className="border border-border rounded-lg p-6 bg-card" data-testid="contacts-panel">
            ...
            {contacts.length === 0 ? (
              ...
              <div className="space-y-2">
                {contacts.map((c) => (
                  <div key={c.contact_id} className="flex items-center justify-between border border-border rounded-md p-3" data-testid={`contact-${c.contact_id}`}>
          ...
          <div className="border border-border rounded-lg p-6 bg-card" data-testid="pack-summary">
        ...
        <div className="border border-border rounded-lg p-6 bg-card h-fit">
          ...
          <div className="mt-4 space-y-2">
            <button onClick={() => exportPack(true)} disabled={exporting} data-testid="export-full" className="w-full flex items-center justify-center gap-2 bg-primary text-primary-foreground py-3 rounded-md font-semibold hover:opacity-90 transition-opacity disabled:opacity-60">
              <DownloadSimple size={16} weight="bold" /> {exporting ? "Building…" : "Full pack + documents"}
            </button>
            <button onClick={() => exportPack(false)} disabled={exporting} data-testid="export-summary" className="w-full flex items-center justify-center gap-2 border border-border py-3 rounded-md font-medium hover:bg-secondary transition-colors disabled:opacity-60">
      ...
      <Modal open={showForm} onClose={() => setShowForm(false)} title="Add trusted contact" testid="contact-form">
            <div className="grid grid-cols-2 gap-3">
              ...
              <Sel label="Relationship" value={form.relationship} onChange={(v) => setF("relationship", v)} options={["spouse", "partner", "child", "parent", "sibling", "friend", "other"]} />
              ...
              <Sel label="Access level" value={form.access_level} onChange={(v) => setF("access_level", v)} options={["full", "financial", "insurance"]} />
            ...
            <div className="flex justify-end gap-2 mt-6">
              ...
              <button onClick={add} data-testid="submit-contact" className="px-4 py-2.5 rounded-md bg-primary text-primary-foreground text-sm font-semibold hover:opacity-90 transition-opacity">Save</button>
            </div>
      </Modal>
    </Page>
  );
...
function Sel({ label, value, onChange, options }) {
  return (
    <div>
      <label className="text-[11px] tracking-[0.1em] uppercase text-muted-foreground">{label}</label>
      <select value={value} onChange={(e) => onChange(e.target.value)} className="mt-1 w-full bg-background border border-input rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring">
